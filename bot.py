...
def admin_check_markup(order_id):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✅ Tasdiqlash", callback_data=f"admin_approve_{order_id}"),
...
def admin_panel(message):
    ...
    c.execute("SELECT COUNT(*) FROM orders WHERE status='pending' OR status='pending_check' OR status='pending_admin'")
...
def confirm_payment(call):
    ...
    # Buyurtma holatini "pending_check" ga o'zgartirish (chek kutilmoqda)
    c.execute("UPDATE orders SET status='pending_check', payment_method='card' WHERE order_id=?", (order_id,))
    conn.commit()
    conn.close()
    ...
    bot.edit_message_text(
        ...
        f"⏳ Endi to'lov chekini (skrinshot yoki chek raqamini) yuboring.\n"
        f"Administrator uni tekshirib, tasdiqlaydi.",
        call.message.chat.id,
        call.message.message_id,
        parse_mode="HTML"
    )
    
    # Foydalanuvchidan chek so'rash
    bot.send_message(user_id, 
        "📸 <b>Iltimos, to'lov chekini yuboring</b> (rasm yoki chek raqami matn shaklida).\n\n"
        "Admin tekshirgandan so'ng hisobingiz to'ldiriladi.",
        parse_mode="HTML")
    
    # Foydalanuvchini chek yuborish holatiga o'tkazish uchun registrator
    bot.register_next_step_handler_by_chat_id(user_id, process_check, order_id)
    bot.answer_callback_query(call.id, "✅ Iltimos, to'lov chekini yuboring.", show_alert=True)

def process_check(message, order_id):
    """Foydalanuvchi yuborgan chekni qabul qilish"""
    user_id = message.from_user.id
    ...
    if not order or order[0] != 'pending_check':
        bot.send_message(user_id, "❌ Buyurtma topilmadi yoki allaqachon ko'rib chiqilgan.", parse_mode="HTML")
        conn.close()
        return
    
    # Buyurtma holatini "pending_admin" ga o'zgartirish
    c.execute("UPDATE orders SET status='pending_admin' WHERE order_id=?", (order_id,))
    conn.commit()
    conn.close()
...
Chek:
    ...
    if message.photo:
        ...
        bot.send_photo(ADMIN_ID, file_id, caption=admin_text, parse_mode="HTML", reply_markup=admin_check_markup(order_id))
    elif message.text:
        # Matn yuborilgan bo'lsa
        admin_text += f"\n{message.text}"
        bot.send_message(ADMIN_ID, admin_text, parse_mode="HTML", reply_markup=admin_check_markup(order_id))
    else:
        bot.send_message(user_id, "❌ Noto'g'ri format. Iltimos, rasm yoki matn yuboring.", parse_mode="HTML")
        return
    
    bot.send_message(user_id, "✅ Chek adminga yuborildi. Tez orada tasdiqlanadi.", parse_mode="HTML")
...
def admin_approve(call):
    ...
    conn = sqlite3.connect('bot_data.db')
    c = conn.cursor()
    c.execute("SELECT user_id, amount, product_type, product_id FROM orders WHERE order_id=?", (order_id,))
    order = c.fetchone()
    if not order:
        bot.answer_callback_query(call.id, "❌ Buyurtma topilmadi!", show_alert=True)
        conn.close()
    ...
    user_id, amount, product_type, product_id = order
    ...
    if product_type == 'topup':
        ...
        pkg = STARS_PRICES[product_id]
        c.execute("UPDATE users SET stars=stars+? WHERE user_id=?", (pkg['count'], user_id))
        success_text = f"✅ {pkg['display']} balansingizga qo'shildi!"
    else:
    ...
    c.execute("""INSERT INTO transactions(transaction_id, user_id, amount, type, description, created_at)
                 VALUES(?, ?, ?, ?, ?, ?)""",
              (transaction_id, user_id, amount, product_type, f"Buyurtma {order_id} tasdiqlandi", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    
    conn.commit()
    conn.close()
    
    # Foydalanuvchiga xabar
    bot.send_message(user_id, f"✅ <b>Buyurtma tasdiqlandi!</b>\n\n{success_text}", parse_mode="HTML")
    ...
    bot.answer_callback_query(call.id, "✅ Buyurtma tasdiqlandi!", show_alert=True)
...
def admin_reject(call):
    ...
    # Foydalanuvchiga xabar
    bot.send_message(user_id, f"❌ <b>Buyurtma rad etildi!</b>\n\nBuyurtma #{order_id}\nSabab: Chek noto'g'ri yoki to'lov amalga oshmagan.\n\nQaytadan urinib ko'ring yoki admin bilan bog'lanishingiz mumkin.", parse_mode="HTML")
    ...
    bot.answer_callback_query(call.id, "❌ Buyurtma rad etildi!", show_alert=True)
...
def admin_confirm(message):
    ...
    conn = sqlite3.connect('bot_data.db')
    c = conn.cursor()
    c.execute("SELECT user_id, amount, product_type, product_id, status FROM orders WHERE order_id=?", (order_id,))
    order = c.fetchone()
    if not order:
        bot.send_message(ADMIN_ID, "❌ Buyurtma topilmadi.", parse_mode="HTML")
        conn.close()
        return
    user_id, amount, product_type, product_id, status = order
    if status not in ['pending', 'pending_check', 'pending_admin']:
    ...
    bot.send_message(ADMIN_ID, f"✅ Buyurtma #{order_id} tasdiqlandi.", parse_mode="HTML")
    bot.send_message(user_id, 
